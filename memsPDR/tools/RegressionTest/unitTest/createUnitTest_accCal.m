function createUnitTest_accCal(varargin)
...
%store in memsPDR/logs/unitTest/accCal. It will create
%unitTest_accCal_xx.txt
...
if(isempty(destdir))
    destdir = sprintf('%s/logs/unitTest/accCal/', MAIN_DIR);
end
...
if(~fileNameDefine)
    ...
    for n = 1:length(testList)
        logfilesAddress(n).dir = sprintf('%s/logs/performanceTest/accCal/Test%02d', MAIN_DIR, testList(n));
...
for n = 1:length(logfilesAddress)
    logFileName = sprintf('%s/unitTest_accCal_%02d.txt', destdir, testList(n));
    ...
    logging.unitTestLogging.accCal = fileID;
    logging.unitTestLogging.memsControl = fileID;
    logging.verbose = 0;
    disp(sprintf('Creating Unit test -  unitTest_accCal_%02d.txt', testList(n)));
    %disp(nameF);
    memsControl = defaultMemsControl;
    memsControl.controlMask = memsConsts.enableAccCal;
