function createUnitTest_magCal(varargin)
...
%store in memsPDR/logs/unitTest/magCal. It will create
...
if(isempty(destdir))
    destdir = sprintf('%s/logs/unitTest/magCal/', MAIN_DIR);
end
...
for n = 1:length(logfilesAddress)
    logFileName = sprintf('%s/unitTest_magCal_%02d.txt', destdir, testList(n));
    ...
    logging.unitTestLogging.magCal = fileID;
    logging.unitTestLogging.memsControl = fileID;
    logging.verbose = 0;
    disp(sprintf('Creating Unit test -  unitTest_magCal_%02d.txt', testList(n)));
    %disp(nameF);
    memsControl = defaultMemsControl;
    memsControl.controlMask = memsConsts.enableMagCal;
