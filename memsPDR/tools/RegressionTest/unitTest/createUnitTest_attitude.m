function createUnitTest_attitude(varargin)
...
%store in memsPDR/logs/unitTest/attitude. It will create
%unitTest_attitude_xx.txt
...
if(isempty(destdir))
    destdir = sprintf('%s/logs/unitTest/attitude/', MAIN_DIR);
end
...
if(~fileNameDefine)
    ...
    for n = 1:length(testList)
        logfilesAddress(n).dir = sprintf('%s/logs/performanceTest/attitude/test%02d', MAIN_DIR, testList(n));
...
for n = 1:length(logfilesAddress)
    logFileName = sprintf('%s/unitTest_attitude_%02d.txt', destdir, testList(n));
    ...
    logging.unitTestLogging.attitude = fileID;
    logging.unitTestLogging.memsControl = fileID;
    logging.unitTestLogging.memsConfig = fileID;
    logging.verbose = 0;
    disp(sprintf('Creating Unit test -  unitTest_attitude_%02d.txt', testList(n)));
    %disp(nameF);
